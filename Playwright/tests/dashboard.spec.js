...
test.describe('Dashboard Page', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.fill('[placeholder="Your email"]', 'bashinisankalpana108@gmail.com');
    await page.fill('[placeholder="Your password"]', 'bashi1234');
  ...
  test('should display user data on the Dashboard', async ({ page }) => {
    ...
    await expect(page.locator('.app-name')).toHaveText('HEALTHPAL ADVISOR');
  });

  test('should navigate to the Streamlit UI for Health Recommendations and stop testing', async ({ page }) => {
    // Increase timeout to handle longer load times
    test.setTimeout(60000); // 60 seconds

    console.log('Attempting to click on Health Recommendation option.');
